...
Arrow_For_Green_Attack::Arrow_For_Green_Attack(double x_center, double y_center, double v_x, double v_y, Direction direction, std::string obj_name, ArrowType arrow_type) : m_v_x(v_x), m_v_y(v_y), m_direction(direction), m_arrow_type(arrow_type) {
    m_obj_name = obj_name;
    m_z_index = 4;

    double height = (Engine::SCREEN_HEIGHT * 0.06);
    double width = height * SPRITE_WIDTH_TO_HEIGHT_RATIO;
...
void Arrow_For_Green_Attack::update() {
    ...
    if (m_state == State::APPROACHING_BEFORE_ROTATE || m_state == State::APPROACHING_AFTER_ROTATE) {
        transform->m_x_center += m_v_x * Engine::get().get_delta_time();
        transform->m_y_center += m_v_y * Engine::get().get_delta_time();
    }
    ...
    if (m_arrow_type == ArrowType::Yellow && m_state == State::APPROACHING_BEFORE_ROTATE) {
        double dist = distance(transform->m_x_center, transform->m_y_center, player_transform->m_x_center, player_transform->m_y_center);

        if (dist <= DISTANCE_FROM_SOUL_TO_FLIP) {
            m_state = State::ROTATING;
            m_orbit_radius = dist;

            double dx = transform->m_x_center - player_transform->m_x_center;
            double dy = transform->m_y_center - player_transform->m_y_center;
    ...
    if (m_state == State::ROTATING) {
        ...
        transform->m_x_center = player_transform->m_x_center + m_orbit_radius * std::cos(orbit_angle_rad);
        transform->m_y_center = player_transform->m_y_center + m_orbit_radius * std::sin(orbit_angle_rad);
...
void Arrow_For_Green_Attack::render() {
    ...
    Engine::get().draw_texture(m_texture, transform->m_x_center, transform->m_y_center, transform->m_width, transform->m_height, angle);
}
