...
void Arrow_For_Green_Attack::update() {
    ...
    if (distance(x_center, y_center, shield->x_center, shield->y_center) <= 20) {
        play_sound_effect("shield_reflect.ogg");
        this->to_be_removed = true;
    }
}

void Arrow_For_Green_Attack::render() {
    SDL_Rect rect{static_cast<int>(x_center - width / 2), static_cast<int>(y_center - height / 2), static_cast<int>(width), static_cast<int>(height)};
